...
func generateInitParams(t *testing.T, r *rand.Rand) *services.VersionedGlobalParams {
	var pks []string
...
func genValidGlobalParam(
	...
	if num == 1 {
		return &services.GlobalParams{
			Versions: []*services.VersionedGlobalParams{initParams},
		}
	}

	var versions []*services.VersionedGlobalParams
...
func FuzzParseValidParams(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		parsedParams, err := services.ParseGlobalParams(globalParams)
		require.NoError(t, err)
